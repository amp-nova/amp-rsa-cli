...
const handle = (settingsJSON) => {
    const output = childProcess.execSync(`./node_modules/.bin/dc-cli indexes list --json`).toString();
    ...
    if (indexes.length > 0) {
        ...
        if (searchKeyLink) {
            ...
            const output = childProcess.execSync(`./node_modules/.bin/dc-cli indexes get-search-api-key ${index.id} ${apiKeyId} --json`).toString();

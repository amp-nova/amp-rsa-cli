...
const handle = (settingsJSON) => {
    const output = childProcess.execSync(`npx @amplience/dc-cli indexes list --json`).toString();
    ...
    if (indexes.length > 0) {
        ...
        if (searchKeyLink) {
            ...
            const output = childProcess.execSync(`npx @amplience/dc-cli indexes get-search-api-key ${index.id} ${apiKeyId} --json`).toString();

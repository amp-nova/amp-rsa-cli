...
const handle = (settingsJSON, argv) => {
    ...
    serverless.map((item) => {
        ...
        childProcess.execSync(`sls info --aws-profile ${settingsJSON.serverless.customProfileName} > deployment.yaml`, { cwd: `./repositories/${item}` });
        childProcess.execSync(`sed -i -e '1,2d' deployment.yaml`, { cwd: `./repositories/${item}` });
        console.log(`Loading serverless service information from ./repositories/${item}`);
        const deploymentYAML = fs_1.readFileSync(`${argv.settingsDir}/repositories/${item}/deployment.yaml`).toString();
